...
function stubApi({ logOutFails = false } = {}) {
  const fetchMock = vi.fn(
    async (_path: string | URL | Request, init?: RequestInit) => {
      if (init?.method === "DELETE") {
        if (logOutFails) throw new TypeError("Network request failed");
        return new Response(null, { status: 204 });
      }

      const { deriveCredentials, generateWrappedDataKey } =
        await import("../lib/crypto");
      const { wrapKey } = await deriveCredentials(EMAIL, PASSWORD, {
        iterations: 1_000,
      });
...
function Harness() {
  ...
  return (
    <div>
      ...
      <button
        type="button"
        onClick={() => void logIn(EMAIL, PASSWORD).catch(() => {})}
      >
        Log in
      </button>
...
describe("AuthProvider", () => {
  ...
  it("sends the same normalized email it salted with", async () => {
    ...
    await userEvent
      .setup()
      .click(screen.getByRole("button", { name: "Log in padded" }));
